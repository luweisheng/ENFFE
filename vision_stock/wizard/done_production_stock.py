...
class DoneProductionStock(models.TransientModel):
    _name = 'done.production.stock'
    _inherit = 'stock.allot.base'
    _description = '完工入库调拨'

    production_ids = fields.Many2many('mrp.production', 'done_production_stock_mrp_ids',
                                      string='生产订单',
                                      domain=[('state', 'in', ('confirmed', 'underway', 'done')), ('surplus_qty', '>', 0)])
    # 入库明细
    done_stock_line_ids = fields.One2many('done.production.stock.line', 'done_production_stock_id', string='完工入库明细')

    @api.onchange('product_qty')
    def _done_production_onchange_product_qty(self):
        # 数量禁止小于0
        if self.product_qty < 0:
            raise UserError(_('数量不能小于0！'))
        self.done_stock_line_ids.done_qty = self.product_qty

    @api.onchange('location_id', 'location_dest_id')
    def _done_production_onchange_location(self):
        for line in self.done_stock_line_ids:
            line.location_id = self.location_id
            line.location_dest_id = self.location_dest_id

    # 调拨方式，入库、退货
    allot_type = fields.Selection([('in', '入库'), ('return', '退库')], string='调拨方式', default='in')
    ...
    def _production_set_stock_line_ids(self):
        ...
        if production_ids:
            ...
            self.done_stock_line_ids = stock_line_ids
...
class DoneProductionStockLine(models.TransientModel):
    _name = 'done.production.stock.line'
    _inherit = 'stock.allot.base.line'
    _description = '完工入库调拨明细'

    done_production_stock_id = fields.Many2one('done.production.stock', string='完工入库调拨')
